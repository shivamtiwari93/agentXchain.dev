...
If you are building a local stdio adapter instead, start with the [Protocol Implementor Guide](./protocol-implementor-guide). If you want the CLI flags, see the [CLI Reference](./cli#verify-protocol).
...
## Minimal conformance server example
...
function executeFixture(fixture) {
  // Map fixture.input.operation onto your implementation
  // Compare result against fixture.expected
  // Return { status, message, actual }
  return { status: 'not_implemented', message: null, actual: null };
}
...
server.listen(8080, () => {
  console.log('Conformance server on http://localhost:8080');
});
```
