...
`paused` is reserved for real blockers like unresolved escalations or blocked runs. A healthy post-completion path should stay `running` while it seeds the next objective, then end as `completed` or `idle_exit` when it hits `max_runs`, budget, or exhausted vision goals.
...
Generated BUG-59 safe-rule configs use `phase_transitions.default: "require_human"` with explicit auto-approve rules. That conservative posture does **not** auto-enable ghost retry. For those projects, opt in with the config above or use `agentxchain run --continuous --auto-retry-on-ghost`.
