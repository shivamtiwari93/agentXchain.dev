...
|---|---|
| `step --resume` | Resume a blocked turn (dispatch failure, escalation, hook block, agent request) — dispatches and runs the agent |
| `resume` | Resume a blocked run: re-dispatches retained turn or assigns the next turn (budget, escalation, any blocked state) |
