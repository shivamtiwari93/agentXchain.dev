...
| State | Cause | Recovery |
|---|---|---|
| `budget_exhausted` | Cumulative turn costs exceeded `per_run_max_usd` | Increase `per_run_max_usd` in `.agentxchain/config.json`, then `agentxchain step --resume` |

The turn that exhausts the budget IS accepted — its work is preserved. Only subsequent turns are blocked until the operator raises the budget limit.
