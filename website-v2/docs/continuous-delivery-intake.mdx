...
`intake resolve` does not restart work by itself. It records the outcome. If the run or workstream blocked and the operator decides to continue, the next step is `agentxchain intake approve --intent <id>` followed by re-planning and a new `intake start` or `intake handoff`.
