...
`manual` is valid for `intake record`. It is intentionally excluded from `intake scan`. `vision_scan` is used by `run --continuous` to record intents derived from VISION.md during vision-driven operation.
...
`intake start` can also handle one non-rejection special case that operators should understand:
