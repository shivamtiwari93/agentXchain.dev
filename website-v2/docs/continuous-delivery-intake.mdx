...
description: "Repo-native intake for continuous governed delivery: record and scan signals, triage work, approve intent, generate planning artifacts, start governed execution, and inspect backlog state."
---
...
The current CLI surface is seven commands:
...
That distinction still matters. The repo continues to store `requires_human_start: true`. Approval and planning prepare work; `intake start` is the explicit human-authorized bridge that turns prepared work into governed execution. `intake scan` only widens ingestion. It does not widen execution authority.
