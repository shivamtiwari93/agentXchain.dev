...
3. **Collision protection** — If a plugin with the same name is already installed, the installer rejects. Hook event collisions (two plugins hooking the same event) are allowed but logged as warnings.
