...
A reproducible CLI proof now exists at `examples/governed-todo-app/run-delegation-proof.mjs`.
...
This matters because the proof goes through the real `agentxchain step` lifecycle, not direct state mutation. It exercises delegation enqueueing, delegated turn routing, delegation review routing, and clean run completion through the actual CLI/operator path.
