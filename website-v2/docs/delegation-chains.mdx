...
| `required_decision_ids` | string[] | Optional named decisions the child turn must emit before parent review may advance phase/run lifecycle |
...
- `required_decision_ids`, when present, must be unique `DEC-NNN` values
...
- **Acceptance contract:**
  ...
  - New JWT validation tests added
  - No breaking changes to API endpoints
- **Required decisions:** DEC-101, DEC-102
...
- **Verification:** pass
- **Required decisions:** DEC-101, DEC-102
- **Satisfied decisions:** DEC-101
- **Missing decisions:** DEC-102
...
Evaluate whether each delegation met its acceptance contract and returned any required named decisions.
```
...
If any delegation result still has **Missing decisions**, the parent review turn **cannot** request a phase transition or run completion. It must route the run back to an implementation role for corrective work.
