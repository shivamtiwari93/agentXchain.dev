...
Timeouts fire at governed acceptance boundaries, not through a background daemon:

1. `agentxchain accept-turn` for mutating enforcement of turn, phase, and run limits
2. `agentxchain status` for proactive read-only visibility of current timeout pressure and persisted timeout blocks

`approve-transition` and `approve-completion` do not currently re-run timeout mutation. The accepted-turn boundary is the authoritative enforcement point today, and the docs must stay honest about that.

That keeps timeout evidence deterministic and tied to governed state transitions instead of background timers.
