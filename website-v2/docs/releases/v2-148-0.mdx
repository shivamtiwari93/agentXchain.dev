...
For closure evidence on `v2.148.0`, quote the exact command output or diagnostic lines for these three checks:
...
The closure artifact is the tester's quoted shipped-package output. No source-tree run, local green test, or agent summary is sufficient for BUG-54 or BUG-55.
