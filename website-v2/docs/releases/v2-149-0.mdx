...
- **BUG-54 QA startup reliability after auth is corrected**: run the tester's normal QA flow on `v2.149.0` and quote the adapter diagnostic lines that include `startup_latency_ms`, `first_output_stream`, `watchdog_fired`, and `exit_signal` on `process_exit`. Closure requires QA dispatches succeeding at `>90%` on the tester's real setup. If QA still fails after env auth or `--bare` is set, quote the full `process_exit` record for a failing attempt.
