...
> For tester closure evidence on still-open bugs, use the latest shipped contract at [`v2.148.0` Tester Re-Run Contract](/docs/releases/v2-148-0#tester-re-run-contract). The BUG-52 and BUG-53 fixes in `v2.147.0` ride along unchanged in `v2.148.0`, so closure proof should be pinned to the latest published package.
