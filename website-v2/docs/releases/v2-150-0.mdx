...
- **BUG-54 new discriminator artifact**: run `node cli/scripts/reproduce-bug-54.mjs --attempts 10 --watchdog-ms 10000 --out /tmp/bug54-v2-150-0.json` inside the failing worktree and quote the following fields from the resulting JSON per `.planning/BUG_54_DISCRIMINATOR_RUNBOOK.md`:
