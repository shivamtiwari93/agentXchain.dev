...
credentialed hard-stop counter-evidence. BUG-53 still requires a real
multi-run chain trace. BUG-61 still requires shipped-package quote-back for
ghost auto-retry and exhaustion diagnostics. `v2.154.0` does not alter those
closure contracts.
