...
Need the current normative boundary instead of the overview? See [Protocol Reference v6](./protocol-reference). Building a non-CLI runner? See [Runner Interface](/docs/runner-interface).
...
Each `step` creates a turn, validates the staged result, and auto-accepts it into history. `accept-turn` exists only for recovery flows (interrupted turns or pre-staged results that need manual acceptance). Gates (`approve-transition`, `approve-completion`) are human-approval checkpoints that the orchestrator cannot bypass.
