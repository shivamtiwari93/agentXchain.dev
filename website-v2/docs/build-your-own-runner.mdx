...
If you are building a CI runner, a hosted orchestrator, or an internal execution service, the rule is simple: import governed execution operations only through `cli/src/lib/runner-interface.js`. If you shell out to `agentxchain step`, you did not build a second runner. You wrapped the CLI.
...
} from '../cli/src/lib/runner-interface.js';
```

Do not import `governed-state.js`, `turn-paths.js`, or other internal helpers directly. That shortcut buys you nothing except future drift.
...
AgentXchain already ships the graduation path. Use it in order instead of jumping straight to abstractions.
...
- Importing internal helpers directly instead of `runner-interface.js`. That is boundary violation, not engineering.
...
- [`examples/ci-runner-proof/README.md`](https://github.com/shivamtiwari93/agentXchain.dev/tree/main/examples/ci-runner-proof) for the runnable proof scripts in one place
