...
3. **Start from clean state only.** Scheduled runs start only from `idle` or `completed` status.
...
## Combining with multi-repo

For multi-repo coordination, run the daemon from the coordinator workspace (rooted by `agentxchain-multi.json`). Scheduled runs in the coordinator can trigger `multi step` commands that fan out to child repos.

See [Multi-Repo Orchestration](/docs/multi-repo) for the coordinator setup.
