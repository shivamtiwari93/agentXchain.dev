...
description: "Copy-paste path from demo to your first real governed run, distinguishing the manual-first generic scaffold from mixed-mode project templates."
---
...
If you want the shortest no-key path to a real accepted turn first, read [Build Your First Governed Project in 5 Minutes](/docs/five-minute-tutorial). If you want artifact-by-artifact detail, read [Your First Governed Turn](/docs/first-turn). This page stays focused on the fastest real command sequence.
...
:::caution Clean working tree required for authoritative turns
Authoritative and proposed turns require a **clean git baseline** — the framework compares `files_changed` in the turn result against the diff from the dispatch baseline. If you have uncommitted changes in actor-owned files when you try to assign a turn, the assignment will fail. Always **commit scaffold and config changes before running your first automated turn**. This is why the scaffold instructions above end with `git add -A && git commit`.
:::
...
- [Build Your First Governed Project in 5 Minutes](/docs/five-minute-tutorial) for the fastest manual-first path to a real accepted PM turn
