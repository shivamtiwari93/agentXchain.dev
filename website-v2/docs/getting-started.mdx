...
**Optional:** add a project goal so every agent turn receives mission context in its dispatch bundle:

```bash
# Either pass --goal at init time:
agentxchain init --governed --template cli-tool --goal "Build a CLI that lints decision logs" --dir . -y

# Or set it later in agentxchain.json under project.goal
```
