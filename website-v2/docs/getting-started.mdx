...
`doctor` checks config, binaries, env vars, and repo state. `connector check` is the live transport probe: it actively checks configured non-manual runtimes before the first turn. If either command fails, fix it before execution starts — it is cheaper to catch missing env vars or dead endpoints now than mid-run.
