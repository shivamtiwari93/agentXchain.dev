...
`doctor` checks config, binaries, env vars, and repo state. `connector check` is the live transport probe: it actively checks configured non-manual runtimes before the first turn and prints `Probing ...` progress lines while it works. If either command fails, fix it before execution starts — it is cheaper to catch missing env vars or dead endpoints now than mid-run. Use `agentxchain connector check --timeout <ms>` if you want a shorter or longer per-connector network budget while debugging.
