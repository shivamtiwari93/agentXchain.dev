...
| **Model support** | OpenAI-first, extensible | Model-agnostic via adapters |
...
AutoGen's coordination lives inside the Python runtime. If you want to use AutoGen's multi-agent model from a different language, IDE, or cloud environment, you need to wrap the Python process.

AgentXchain's protocol is language-independent. The governed state machine, turn contracts, and decision ledger are defined as JSON schemas. Any runtime that speaks the protocol can participate — Python, TypeScript, shell scripts, cloud APIs, IDE extensions.
