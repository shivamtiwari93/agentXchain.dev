...
- The published schema also documents the runner-owned watchdog knobs `run_loop.startup_watchdog_ms` and `run_loop.stale_turn_threshold_ms`, so BUG-51/BUG-47 tuning is part of the machine-readable config contract instead of an implementation-only detail.
