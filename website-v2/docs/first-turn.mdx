...
You ran `npx --yes -p agentxchain@latest -c "agentxchain demo"` and saw governance in action. Now do it for real on your own repo. This guide walks through every step, shows you what each artifact looks like, and tells you exactly when to act.
...
agentxchain init --governed --template cli-tool --goal "Build a governed CLI tool" --dir . -y
...
├── .agentxchain/
│   ├── state.json                # ← gitignored by default (runtime state, durable on disk)
│   ├── history.jsonl             # ← gitignored by default (runtime state, durable on disk)
│   ├── decision-ledger.jsonl     # ← gitignored by default (runtime state, durable on disk)
│   ├── dispatch/turns/           # ← gitignored (transient)
│   ├── staging/                  # ← gitignored (transient)
│   ├── reviews/                  # ← gitignored (transient)
│   └── prompts/
│       ├── pm.md                 # ← commit this
│       ├── dev.md                # ← commit this
│       ├── qa.md                 # ← commit this
│       └── eng_director.md       # ← commit this
├── .planning/
│   ├── PM_SIGNOFF.md             # ← commit this (starts at "Approved: NO")
│   ├── ROADMAP.md                # ← commit this
│   ├── SYSTEM_SPEC.md            # ← commit this
│   ├── IMPLEMENTATION_NOTES.md   # ← commit this
│   ├── acceptance-matrix.md      # ← commit this
│   ├── ship-verdict.md           # ← commit this
│   ├── RELEASE_NOTES.md          # ← commit this
│   └── (template extras)         # e.g. command-surface.md for cli-tool
└── TALK.md                       # ← gitignored by default (runtime state, durable on disk)
```
...
## Inspect The Run Honestly

Do not collapse every proof surface into a vague "report the run" mental model. AgentXchain keeps three distinct evidence commands:
...
- `audit` reads the **live current repo or workspace** and renders the governance report immediately.
- `export` writes the **portable artifact** you can archive, diff, share, or verify elsewhere.
- `report --input` reads an **existing export artifact**. It does not inspect live repo state directly.

That same boundary still holds later when you move beyond a single repo into coordinator workspaces. Partial artifacts are valid evidence, not corruption: `audit` and `report` keep `repo_ok_count` / `repo_error_count`, preserve the failed repo row plus error, and do not fabricate failed-child drill-down when a nested child export is missing.
...
| `qa_ship_verdict` failed | `ship-verdict.md` still says `PENDING`, `RELEASE_NOTES.md` is missing, or acceptance evidence is incomplete | Set `## Verdict: YES` (or `NO SHIP` to block), finish `RELEASE_NOTES.md`, and make sure the acceptance matrix is real |
...
- [Adapters](/docs/adapters) — configure all five shipped adapter paths (`manual`, `local_cli`, `api_proxy`, `mcp`, `remote_agent`)
