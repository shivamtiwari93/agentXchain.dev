...
Because `audit` reuses the same report builder as `report`, blocked governed runs keep the same additive operator fields, and coordinator workspaces keep the same ordered recovery surface for `pending_gate`, `repo_run_id_mismatch`, and `multi resync` drift cases:
