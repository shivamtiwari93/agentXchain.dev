...
Completed coordinator workspaces follow the same terminal contract as `report`: if child repo run-id drift remains visible after the coordinator is already `completed`, `audit` keeps `subject.run.run_id_mismatches` visible, populates `subject.run.terminal_observability_note`, and omits `subject.run.next_actions`. Text and markdown output render `Terminal drift note: ...` instead of implying recovery work still exists.
