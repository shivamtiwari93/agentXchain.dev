...
For coordinator workspaces, that shared report contract also keeps the repo-status truth boundary explicit: `summary.repo_run_statuses` remains raw coordinator snapshot metadata, while `subject.run.repo_status_counts` and `subject.run.repo_status_drifts` use authority-first child repo status whenever child authority is readable.
