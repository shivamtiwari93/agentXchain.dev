...
agentxchain mission plan launch [plan_id|latest] --workstream <id> [--mission <mission_id>] [--json] [--dir <path>]
...
# Launch a ready workstream from the approved plan
agentxchain mission plan launch latest --workstream ws-alignment
...
Launch is per-workstream and dependency-aware. `mission plan launch --workstream <id>` validates that the plan is approved and the target workstream's dependencies are satisfied (all depended-on workstreams must have completed chains). Launch records bind `workstream_id` → `chain_id` and attach the chain to the parent mission. If a launched workstream's chain fails, the workstream is marked `needs_attention` and dependent workstreams remain blocked.
