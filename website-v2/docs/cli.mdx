...
| `--dev-command <parts...>` | `claude --print` | Override the scaffolded `local-dev.command` array for the authoritative dev runtime |
...
Intake is repo-local. Run it inside a governed project rooted by `agentxchain.json`. If you are standing in a coordinator workspace rooted by `agentxchain-multi.json`, use `multi` there and run intake inside the child repo that owns the signal.
