...
| `mission show` | Observability | Show the latest mission or one mission by ID, including attached chain summaries |
...
| `mission plan launch` | Planning | Launch a workstream from an approved plan, recording chain linkage |
...
Mission summaries derive aggregate chain count, total runs, total turns, latest chain terminal reason, and active repo-decision count. That last field matters: repo decisions already carry across chained runs, and the mission surface keeps that continuity visible instead of pretending each chain starts from zero.
...
# Launch and execute a ready workstream from the approved plan
...
The planner uses LLM-assisted one-shot generation with strict schema validation. Configure `mission_planner` or `api_proxy` in `agentxchain.json` with `base_url` and `model`. Malformed planner output fails validation closed — no partially-valid plan artifacts are created.
...
Launch is per-workstream and dependency-aware. `mission plan launch --workstream <id>` validates that the plan is approved and the target workstream's dependencies are satisfied, records a preallocated `workstream_id` → `chain_id` launch record, and then executes the workstream through the existing governed run/chaining path using that same chain ID. Dependency satisfaction is based on the depended-on chain's latest run status, not only the chain-loop terminal label. `--auto-approve` passes through to the governed run surface. Successful execution marks the workstream `completed` and attaches the emitted chain report to the parent mission; failed execution marks the workstream `needs_attention` and leaves dependent workstreams blocked.
...
| **Mission** | Repo-local single-repo hierarchy above chains from `/api/missions` — latest mission identity, goal, derived status, aggregate chain/run/turn counts, active repo-decision count, attached-chain lineage summaries, and newest-first recent missions without colliding with coordinator **Initiative** |
