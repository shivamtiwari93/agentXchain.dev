...
- restart guidance (`agentxchain restart`) when the run is restartable
...
The runtime automatically writes checkpoint markers at every governance boundary (turn acceptance, phase transition, run completion). When `restart` runs, it:

1. Reads the last checkpoint and current state
2. Validates the run is resumable (not completed, failed, or blocked)
3. Reactivates the run if paused
4. Assigns the next turn from routing (or reconnects to an abandoned active turn)
5. Writes `.agentxchain/SESSION_RECOVERY.md` — a briefing document for the new session with run identity, checkpoint state, recent decisions, and turn history
...
The shipped dashboard surface has nine top-level views:
...
| **Timeline** | Repo-local run timeline with continuity status, active turns, accepted history, and expandable turn-level hook evidence |
