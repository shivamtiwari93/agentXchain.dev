...
Raises an operator-controlled escalation and moves the run to `status = blocked`. This command is active-run-only. If exactly one active turn exists, that turn is retained automatically and the default recovery action becomes `agentxchain step --resume`. If no turn is active, the escalation blocks the run at the run level and the default recovery action becomes `agentxchain step`. If multiple active turns exist, `--turn` is required so the operator is explicit about which turn the escalation is retaining.
