...
| `benchmark` | Setup | Run a governed delivery compliance proof in baseline or `--stress` mode, and use `--output <dir>` to persist export-proof artifacts for `verify export` and `verify diff` |
...
| `--stress` | `false` | Run the rejected-turn recovery workload instead of the baseline happy path |
...
agentxchain benchmark --output ./artifacts/benchmark-baseline
agentxchain verify export --input ./artifacts/benchmark-baseline/run-export.json

# compare the happy path against the adversarial retry workload
agentxchain benchmark --stress --output ./artifacts/benchmark-stress
