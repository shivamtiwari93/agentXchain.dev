...
| `ci-report` | Inspection | Report governed run results in CI-native formats (GitHub Actions annotations, JUnit XML) |
...
| `reconcile-state` | Turn lifecycle | Accept safe fast-forward operator commits as the new governed baseline after checkpoint drift |
...
| `migrate-intents` | Migration | Archive legacy and phantom intents that block continuous mode |
...
| `ship-status` | Governance | Evaluate whether the project is ready to ship across five evidence dimensions |
| `attention` | Governance | Show the cross-run human-decision queue — what needs you, and nothing else (govern by exception) |
...
| `mission bind-coordinator` | Coordination | Bind an existing coordinator super_run to a mission (requires `--super-run-id`; optional `--coordinator-config`, `--coordinator-workspace`) |
...
| `connector capabilities` | Readiness | Show the merged capability contract for one runtime or all configured runtimes |
...
| `workflow-kit describe` | Readiness | Show the workflow kit contract — templates, artifacts, semantic validators, gate coverage |
| `serve-mcp` | Integration | Start an MCP server exposing governance tools for Claude Code, Cursor, and other MCP clients |
| `serve` | Integration | Start the hosted runner HTTP server for one or more governed projects |
...
agentxchain mission bind-coordinator [mission_id] --super-run-id <id> [--coordinator-config <path>] [--coordinator-workspace <path>] [--json] [--dir <path>]
```
...
`mission bind-coordinator` links a mission to an existing multi-repo coordinator `super_run`. Pass the required `--super-run-id`, plus `--coordinator-config` / `--coordinator-workspace` when the coordinator workspace is not auto-discoverable, so a single-repo mission can track coordinator-driven work.
...
| `--dev-command <parts...>` | `claude --print --dangerously-skip-permissions --bare` | Override the scaffolded `local-dev.command` array for the authoritative dev runtime |
...
```bash
agentxchain ci-report [--input <path>] [--format auto|github-actions|junit-xml|json]
```

| Flag | Default | Description |
|------|---------|-------------|
| `--input <path>` | current project | Read a run export artifact from a file; omit to report on the current governed project |
| `--format <format>` | `auto` | Output format: `auto` (detect CI environment), `github-actions` (inline annotations), `junit-xml`, or `json` |

`ci-report` renders governed run results in CI-native formats so a pipeline can surface gate, verification, and acceptance outcomes without parsing the raw export. With `--format auto` it detects the CI environment (e.g. GitHub Actions) and emits the matching format; use `junit-xml` to publish results to a test-report viewer, or `json` for custom tooling.
...
| `--no-checkpoint` | `false` | Skip git checkpoint after acceptance (workspace stays dirty) |
...
| `continuous.idle_expansion.sources` | VISION, ROADMAP, SYSTEM_SPEC | Source files for PM idle-expansion when `on_idle` is `perpetual` |
| `continuous.idle_expansion.max_expansions` | `5` | Max consecutive PM idle-expansion attempts before `vision_expansion_exhausted` |
| `continuous.idle_expansion.role` | `"pm"` | Role assigned to the PM idle-expansion turn |
| `continuous.idle_expansion.malformed_retry_limit` | `1` | Reserved retry budget for malformed idle-expansion outputs |
...
| `--normalize-staged-result` | `false` | Run known-safe staged-result field normalization before acceptance |
...
| Flag | Default | Description |
|------|---------|-------------|
| `--accept-operator-head` | required | Accept safe fast-forward operator commits on top of the last checkpoint as the new governed baseline |
...
Approves a pending phase transition gate. The run must be in `blocked:gate` state with a pending `phase_transition_request`. On approval, the phase advances (e.g., `planning` to `implementation`, `implementation` to `qa`).
...
|------|---------|-------------|
| `-j, --json` | `false` | Output as JSON |
| `--dry-run` | `false` | List legacy intents without modifying them |
...
| Flag | Default | Description |
|------|---------|-------------|
| `[runtime_id]` | none | Show one configured runtime's merged capability contract |
| `--all` | `false` | Show all configured runtimes |
...
- `source` — `default`, `explicit`, or `mixed`
...
The output shape is governed by a published JSON Schema at `agentxchain/schemas/workflow-kit-output`. See the [Workflow Kit](/docs/workflow-kit) page for the full operating model.
...
| View | Description |
|------|-------------|
| **Org Overview** | Organization-scoped first-glance summary across registered projects from `/v1/org/overview` — aggregate metric cards (projects, active runs, pending gates, total decisions, total cost) plus a per-project card grid with status, phase, run id, active turns, cost, pending gates, and any read error |
| **Org Runs** | Cross-project run list from `/v1/org/runs` as a filterable table by project, phase, and status — per-run project name, run id, phase, status, turns completed, cost, and relative started/updated times |
| **Org History** | Full-fidelity cross-project run history from `/v1/org/history` — newest-first run cards with project name, status, duration, total cost, completed phases, retrospective headline, and per-gate pass/pending results, plus completed/blocked roll-up counts |
| **Audit Trail** | Unified organization governance audit trail from `/v1/org/audit-trail` — severity- and source-tagged events (decision ledger, hook audit, events) filterable by project, severity, and source, with event type, project, summary, timestamp, and phase/role/run context |
...
| **Notifications** | Configured webhook targets plus recent delivery truth from `/api/notifications` — attempts, failures, timeouts, approval SLA reminder config, and newest-first rows from `.agentxchain/notification-audit.jsonl` |
...
| **Watch** | Watch Mode intake audit from `/api/watch-results` — recent webhook, event-file, and event-directory results with route status, delivery ID, event/intent IDs, run/role details, deduplication state, and errors from `.agentxchain/watch-results/*.json` |
...
|------------|---------|--------------|
| `record` | Record one delivery trigger event and create a detected intent | Accepts inline, file, or stdin payloads. Supported sources: `manual`, `ci_failure`, `git_ref_change`, `schedule`, `vision_scan`, `vision_idle_expansion` |
