...
`--json` returns structured turn metadata by default. When combined with `--artifact assignment` or `--artifact manifest`, it returns the parsed JSON artifact content instead of raw filesystem output.
...
`report` turns a verified export artifact into a concise governance summary for operators, reviewers, and release surfaces. It verifies the export artifact first and fails closed on invalid artifacts instead of pretending an untrusted JSON blob is audit evidence. Governed run reports summarize project identity, run status, phase, blocked state, turn counts, budget, and evidence counts. When a governed project has `project.goal`, text/markdown report output renders `Goal: <text>` in the header and `--format json` exposes the same value at `subject.project.goal`. Coordinator reports summarize workspace identity, repo/workstream/barrier counts, repo status breakdown, and per-repo export health.
...
On a valid staged result, `step` auto-accepts the turn immediately. Use `accept-turn` directly when you are fixing a retained failed or conflicted turn, or when you intentionally interrupted a manual step before the CLI detected the staged result.
