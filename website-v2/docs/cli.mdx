...
`multi status` renders coordinator observability with the same depth as single-repo `status`: elapsed time for active runs, blocked reason with ordered recovery guidance when the coordinator is blocked, phase gate pass/pending status, and a completion marker with timestamp when the run finishes. In `--json` mode, the output includes `blocked_reason`, `created_at`, `updated_at`, `phase_gate_status`, and `next_actions` alongside the existing super-run, repo-run, and barrier data.
