...
If the gate has `gate_actions` configured, they execute sequentially after approval hooks pass. A failing gate action blocks the run while preserving the pending gate for retry.
