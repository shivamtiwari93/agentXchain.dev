...
Prints the current run state: active phase, pending turn (if any), blocked status, last accepted turn summary, and recent decision entries. Governed status also surfaces the selected template in both modes: human-readable status prints `Template: <id>`, while `status --json` exposes a top-level `template` field plus `config.template`.
...
`report` turns a verified export artifact into a concise governance summary for operators, reviewers, and release surfaces. It verifies the export artifact first and fails closed on invalid artifacts instead of pretending an untrusted JSON blob is audit evidence. Governed run reports summarize project identity, run status, phase, blocked state, turn counts, budget, and evidence counts. Coordinator reports summarize workspace identity, repo/workstream/barrier counts, repo status breakdown, and per-repo export health.
...
The default table includes a `Trigger` column when provenance metadata is available on the recorded run. Older history entries created before provenance recording show `legacy` instead of inventing a trigger value retroactively.

**Lineage view:** `--lineage <run_id>` walks the `parent_run_id` chain backwards through `run-history.jsonl` and prints the ancestor tree (oldest first). If a parent reference points to a run that no longer exists in history, the chain terminates with a `[broken link]` sentinel. Combine with `--json` for machine-readable lineage output.

Use `--json` for machine-readable output. Use `--status completed` to see only successful runs. Use `--limit 5` to see the last 5 runs.
...
| **Run History** | Cross-run history showing status, trigger, phases, turns, cost, and duration for all governed runs in this project from `/api/run-history` |
