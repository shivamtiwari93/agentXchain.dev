...
| `benchmark` | Setup | Run a governed delivery compliance proof in named workloads (`baseline`, `stress`, `completion-recovery`) and use `--output <dir>` to persist export-proof artifacts for `verify export` and `verify diff` |
...
| `--workload <name>` | `baseline` | Run a named workload: `baseline`, `stress`, or `completion-recovery` |
