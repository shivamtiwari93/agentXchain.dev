...
Shows a persistent cross-run history of governed runs in this project. Each completed governed run appends a summary record to `.agentxchain/run-history.jsonl` with run ID, status, phases, turns, cost, duration, and connector metadata. The file persists across runs and is not reset when a new run starts.

The default table includes a `Trigger` column when provenance metadata is available on the recorded run, plus a `Ctx` column showing whether the run has a usable inheritance snapshot for `--inherit-context` child runs (`✓` = usable snapshot, `—` = metadata-only lineage). Older history entries created before provenance recording show `legacy` instead of inventing a trigger value retroactively.
...
| **Run History** | Cross-run history showing status, trigger, context inheritance availability, phases, turns, cost, and duration for all governed runs in this project from `/api/run-history` |
