...
#### 3. Launch one approved workstream
...
Launches one approved workstream at a time. It validates the approval boundary and dependency satisfaction, records a preallocated `workstream_id -> chain_id` launch record, executes the workstream immediately through the governed run path, and reconciles the workstream outcome from the real chain result.
