...
- Coordinator `--retry` is deliberately narrow: it only supports one active retryable repo failure at a time, it fails closed when a dependent workstream has already dispatched since the failed repo turn, and it does **not** add unattended `--auto-retry` to `mission plan autopilot`.
...
5. Return to the mission surface and resume coordinator execution with either `agentxchain mission plan launch latest --workstream <id>` for a targeted restart or `agentxchain mission plan autopilot latest --continue-on-failure` to continue waves after the repo-local fix.

Coordinator workstreams now support targeted `--retry`, but autopilot still has no unattended coordinator `--auto-retry` surface. Retry remains operator-initiated.
