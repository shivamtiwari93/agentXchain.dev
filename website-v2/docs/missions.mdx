...
Coordinator retry stays manual on purpose. When a repo-local dispatch inside a coordinator workstream fails, the workstream becomes `needs_attention`, `mission plan show` exposes `repo_failures[]`, and autopilot either stops (`failure_stopped`) or exits `plan_incomplete` after exhausting other launchable work.
...
3. Repair the repo-local problem using the normal governed recovery surface for that repo:
...
4. Return to the mission surface and resume coordinator execution with either `agentxchain mission plan launch latest --workstream <id>` for a targeted restart or `agentxchain mission plan autopilot latest --continue-on-failure` to continue waves after the repo-local fix.

Coordinator workstreams do **not** support `--retry`. The retry boundary remains repo-local until a separate coordinator retry contract exists.
