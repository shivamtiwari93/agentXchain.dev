...
5. If the coordinator enters `blocked`, fix the cause and run `agentxchain multi resume`
   - `multi resume` always resyncs from repo authority first
...
- Use `agentxchain multi resume` only after the blocked cause is fixed and you want to clear coordinator `status: "blocked"`
