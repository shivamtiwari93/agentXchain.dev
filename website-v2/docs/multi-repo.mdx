...
`interface_alignment` is explicit, not magical. The coordinator does not infer semantic compatibility from changed files. You declare the required accepted decision IDs per repo in `agentxchain-multi.json`:
