...
The latest checked-in proof artifact is [`examples/live-governed-proof/evidence/multi-repo-proof.latest.json`](https://github.com/shivamtiwari93/agentXchain.dev/blob/main/examples/live-governed-proof/evidence/multi-repo-proof.latest.json).
...
agentxchain intake record \
  ...
  --signal '{"description":"payment flow regressed across services"}' \
  --evidence '{"type":"text","value":"integration suite failing on checkout"}'
...
agentxchain intake handoff --intent <intent_id> --coordinator-root ../workspace --workstream payment_flow
```
