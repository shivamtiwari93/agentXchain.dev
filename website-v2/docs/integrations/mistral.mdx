...
cd my-project
# Replace the scaffolded runtime wiring in agentxchain.json with the Mistral config above.
agentxchain doctor
agentxchain connector check
agentxchain connector validate mistral-dev
agentxchain run
```

If you prefer the guided interactive scaffold, run `agentxchain init --governed` without `-y`, then update `agentxchain.json` with the Mistral config above before `agentxchain connector check` and `agentxchain connector validate mistral-dev`.
