...
agentxchain connector validate mlx-dev
agentxchain run
```

If you prefer the guided interactive scaffold, run `agentxchain init --governed` without `-y`, then update `agentxchain.json` with the MLX config above before `agentxchain connector check` and `agentxchain connector validate mlx-dev`.
