...
If you prefer the guided interactive scaffold, run `agentxchain init --governed` without `-y`, then update `agentxchain.json` with the MCP config above before `agentxchain connector check`.
