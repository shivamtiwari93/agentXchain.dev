...
cd my-project
# Replace the scaffolded runtime wiring in agentxchain.json with the Anthropic config above.
agentxchain doctor
agentxchain connector check
agentxchain connector validate claude-qa
agentxchain run
```

If you prefer the guided interactive scaffold, run `agentxchain init --governed` without `-y`, then update `agentxchain.json` with the Anthropic config above before `agentxchain connector check` and `agentxchain connector validate claude-qa`.
...
```json
{
  ...
  "roles": {
    "architect": { "runtime": "opus-architect", "mandate": "Design system architecture", "write_authority": "review_only" },
    "dev": { "runtime": "sonnet-dev", "mandate": "Implement features", "write_authority": "proposed" },
    "qa": { "runtime": "haiku-qa", "mandate": "Review and test", "write_authority": "review_only" }
  }
}
```
