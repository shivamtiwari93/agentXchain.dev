...
If you want the constitutional model, start with [Protocol v6](./protocol). If you want the executable adoption contract, start here.
...
Fields that are currently informative rather than hard-enforced:

| Field | Current role |
| --- | --- |
| `version` | Human-readable implementation version |
| `surfaces` | Capability metadata for humans/tooling. The verifier does not yet reject a target for missing per-surface declarations. |
| `metadata` | Display metadata only |

Do not overstate what the verifier checks. Today it hard-requires tiers plus the adapter contract. Surface claims are useful metadata, not yet a strict gate.
