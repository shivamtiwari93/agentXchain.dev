...
```json
{
  ...
  "verification": {
    ...
    "produced_files": [
      { "path": "tests/fixtures/output.json", "disposition": "artifact" }
    ]
  },
...
### Verification-produced files are part of the contract

If your verification step creates repo files, declare them explicitly in `verification.produced_files`.

- Use `disposition: "artifact"` when the file is a legitimate output of the turn and should stay in the repo as part of the accepted artifact set.
- Use `disposition: "ignore"` when the file is only a temporary verification side effect and should be restored to the dispatch baseline before acceptance completes.
...
```json
{
  "verification": {
    "status": "pass",
    "commands": ["node scripts/generate-fixture.mjs", "npm test"],
    "evidence_summary": "Fixture regenerated and tests passed.",
    "machine_evidence": [
      { "command": "node scripts/generate-fixture.mjs", "exit_code": 0 },
      { "command": "npm test", "exit_code": 0 }
    ],
    "produced_files": [
      { "path": "tests/fixtures/api-response.json", "disposition": "artifact" },
      { "path": "tmp/verification.log", "disposition": "ignore" }
    ]
  }
}
```

Do not leave verification-created files undeclared. That is how you strand acceptance/checkpoint/resume: acceptance sees an incomplete turn contract, `checkpoint-turn` has nothing coherent to commit, and `resume` still sees dirty actor-owned files.
...
```bash
agentxchain init --governed --goal "Prove a custom connector against the governance pipeline" --dir . -y
agentxchain doctor
```
...
For scalar follow-up edits after the initial setup, prefer the governed CLI surface instead of hand-editing JSON again. For example:

```bash
agentxchain config --set roles.dev.runtime my-connector
agentxchain config --set runtimes.my-connector.timeout_ms 45000
```
