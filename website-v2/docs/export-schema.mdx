...
| `workflow_phase_order` | Ordered array of phase names from the project's `routing` config, or `null` when no routing is defined. Used by `diff --export` for phase-aware regression detection |
...
| `workflow_phase_order` | Ordered array of phase names from the coordinator's `routing` config, or `null` when no routing is defined |
...
- `summary.dashboard_session` schema validation — verifies status enum, PID/URL/started_at types, and status-consistent invariants (e.g., `running` requires a PID, `not_running` requires all-null fields)
