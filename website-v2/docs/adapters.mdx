...
The local_cli adapter spawns a local subprocess and feeds it the dispatch bundle. The default scaffold targets Claude Code (`claude --print` with `stdin` prompt transport). Any local CLI tool that can accept a prompt and produce a valid turn result works — configure it at scaffold time with `--dev-command` and `--dev-prompt-transport` instead of hand-editing JSON afterward.
