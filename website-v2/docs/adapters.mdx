...
That restriction is operational, not academic. An `api_proxy` QA turn can return a structured verdict and request completion, but it cannot directly author `.planning/acceptance-matrix.md`, `.planning/ship-verdict.md`, or `.planning/RELEASE_NOTES.md`. If your completion gate depends on repo-local QA files, those files must already contain real content from a writable or manual path. The orchestrator can materialize a review artifact under `.agentxchain/reviews/`, but it does not patch your planning docs for you.
