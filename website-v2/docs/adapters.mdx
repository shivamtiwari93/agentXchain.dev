...
- `project_root`, `dispatch_dir`, `assignment_path`, `prompt_path`, `context_path`, `staging_path`
- `prompt` and `context` as rendered markdown strings

The tool must return a valid turn-result object. The adapter stages it, then the normal orchestrator validation and acceptance flow runs unchanged.
