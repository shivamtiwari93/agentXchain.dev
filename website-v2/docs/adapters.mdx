...
| **Best for** | PM review, human QA, approvals | Claude Code (default), or any local CLI via `--dev-command` | Local MCP-based agent runtimes with an explicit governed-turn tool | Direct Anthropic, OpenAI, Google Gemini, or Ollama (local) review calls | External agent services, hosted agent platforms, cross-network governed execution |
