...
Only Anthropic is supported in v1. Additional providers may be added behind the same interface in future versions.
...
| **Cost tracking** | N/A | N/A | Automatic (tokens + USD from provider telemetry) |
| **Write authority** | Any | Any | `review_only` only (v1) |
| **Best for** | PM review, human QA, approvals | Claude Code, Codex, Aider | Direct LLM review calls, batch processing |
