...
- Operators manage proposals with `agentxchain proposal list|diff|apply|reject`. `proposal apply <turn_id>` copies proposed files into the workspace and records a decision-ledger entry. `proposal reject <turn_id> --reason "..."` marks the proposal as rejected. Selective apply is supported with `--file <path>`.
:::
