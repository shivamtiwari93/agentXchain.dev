...
sidebar_position: 4
title: Adapters
description: "Five adapters, one contract. manual, local_cli, mcp, api_proxy, and remote_agent explained."
---
...
| Google | Supported for Gemini models via the generateContent API (for example `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.0-flash`) |

Anthropic, OpenAI, and Google are supported. OpenAI support uses the Chat Completions endpoint with JSON output. Google support uses the Gemini `generateContent` endpoint with `responseMimeType: "application/json"`. The API key is passed as a query parameter per Google's API convention. When Gemini blocks a prompt or halts with a non-`STOP` finish reason, AgentXchain preserves that Google-specific reason in the extraction-failure message instead of collapsing it into a generic parse error. Tool use, background execution, and streaming are out of scope for `api_proxy`.
...
**Supported models with built-in cost tracking:** `claude-sonnet-4-6`, `claude-opus-4-6`, `claude-haiku-4-5-20251001`, `gpt-4o`, `gpt-4o-mini`, `gpt-4.1`, `gpt-4.1-mini`, `gpt-4.1-nano`, `o3`, `o3-mini`, `o4-mini`, `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.0-flash`. Models not in this list still work but report `$0` cost in turn telemetry. Operators can override or extend cost rates via `budget.cost_rates` in `agentxchain.json`.
...
- **`api_proxy`** — speaks Anthropic/OpenAI/Google provider APIs; `remote_agent` speaks the AgentXchain turn-result contract directly
...
| Feature | manual | local_cli | mcp | api_proxy | remote_agent |
|---------|--------|-----------|-----|-----------|--------------|
| **Automation** | None — human does all work | Full — subprocess runs autonomously | Full — MCP tool call runs autonomously | Full — API call runs autonomously | Full — HTTP call runs autonomously |
| **Latency** | Minutes (human speed) | Seconds to minutes | Seconds to minutes | Seconds | Seconds to minutes |
| **Cost** | Human time | Tool license + compute | Tool/runtime dependent | API tokens | Remote service dependent |
| **Prompt transport** | N/A (human reads files) | `argv`, `stdin`, or `dispatch_bundle_only` | MCP stdio `tools/list` + `tools/call` | HTTP request body | HTTP POST with turn envelope |
| **Retry logic** | N/A | Exit code based | None in v1 | Configurable exponential backoff with jitter | None in v1 — retry via `step --resume` |
| **Timeout handling** | Configurable poll timeout | SIGTERM → 10s grace → SIGKILL | Request timeout via MCP client call | HTTP timeout + retry policy | Configurable `timeout_ms` (default 120s) |
| **Cost tracking** | N/A | N/A | N/A | Automatic token tracking; USD when the model has a pinned rate table | N/A — remote service reports cost in turn result |
| **Write authority** | Any | Any | Any | `review_only` or `proposed` only | `review_only` or `proposed` only |
| **Best for** | PM review, human QA, approvals | Claude Code (default), or any local CLI via `--dev-command` | Local MCP-based agent runtimes with an explicit governed-turn tool | Direct Anthropic, OpenAI, or Google Gemini review calls | External agent services, hosted agent platforms, cross-network governed execution |
