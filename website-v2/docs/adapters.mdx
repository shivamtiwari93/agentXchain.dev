...
- **`api_proxy`** — speaks Anthropic/OpenAI provider APIs; `remote_agent` speaks the AgentXchain turn-result contract directly
...
| **Best for** | PM review, human QA, approvals | Claude Code (default), or any local CLI via `--dev-command` | Local MCP-based agent runtimes with an explicit governed-turn tool | Direct Anthropic or OpenAI review calls | External agent services, hosted agent platforms, cross-network governed execution |
