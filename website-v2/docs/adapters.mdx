...
The `mcp` adapter connects to a local Model Context Protocol server over stdio and calls one explicit governed-turn tool.

:::info v1 scope
This slice is **stdio only** and **tool-contract based**. AgentXchain does not claim that any arbitrary MCP server can execute a governed turn. The runtime is valid only when the target server exposes the configured tool (default: `agentxchain_turn`) and that tool returns a valid AgentXchain turn result.
:::

### How it works

1. The orchestrator writes the dispatch bundle.
2. The adapter spawns the configured MCP stdio server.
...
| `command` | yes | Executable to spawn (string or string array) |
...
| `cwd` | no | Working directory for the server process (relative to project root) |

### Example server

A reference MCP server is available at [`examples/mcp-echo-agent/`](https://github.com/shivamtiwari93/agentXchain.dev/tree/main/examples/mcp-echo-agent). It implements the full `agentxchain_turn` tool contract and returns a validator-clean no-op governed turn result. For a real governed-project wiring path, see the MCP dev variant in [`examples/governed-todo-app/README.md`](https://github.com/shivamtiwari93/agentXchain.dev/tree/main/examples/governed-todo-app#mcp-dev-variant). Use the echo server as a starting point for building your own governed MCP agent.
