...
AgentXchain ships governed templates across three categories: manual-first baseline, project-type scaffolds, and automation-pattern blueprints. This page helps you decide.
...
| **Automation-pattern template** | `full-local-cli` | PM, Dev, QA, and Director all start on `local_cli` | No extra planning files; runtime/authority topology changes at init time | Human-gated automation where all governed turns are automated |
...
`full-local-cli` is different. It is not about product shape. It is about runtime posture: all governed turns are automated, but humans still approve the major gates. See [Automation Patterns](/docs/automation-patterns) for the exact operator flow.
...
| `full-local-cli` | None | Automated PM planning with human approvals preserved | Automated QA and Director intervention without manual runtime roles | Human-gated automation loop proven before lights-out operation |
