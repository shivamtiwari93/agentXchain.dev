...
You should see a version number (the installed `agentxchain` release). If you only want the zero-install demo path instead of a real install, use:
...
Verify the scaffold is structurally valid and the environment is ready:
...
The `cli-tool` scaffold used here is mixed-mode:
...
Use `agentxchain config --set` to rebind both roles:
...
Both `manual-dev` and `manual-qa` already exist in the scaffold. These commands only rebind the active roles.
...
You should see `status: completed` and all three phases closed. `audit` is the live current repo/workspace summary surface.

If you need a portable artifact for handoff, export the run and then generate the governance report from that artifact:
...
Do not collapse those commands into one vague "report the run" story. `report --input` reads an existing export artifact; it does not inspect live repo state directly. This produces a structured summary of the run: turns, decisions, gate evaluations, timing, and the complete decision ledger.

That same boundary applies in coordinator workspaces. Partial artifacts are still valid evidence surfaces: `audit` and `report` keep `repo_ok_count` / `repo_error_count`, preserve the failed repo row plus error, and do not fabricate failed-child drill-down when the nested child export is unavailable.
