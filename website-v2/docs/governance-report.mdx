...
`subject.run.repo_status_counts` and `subject.run.repo_status_drifts` do **not** reuse `summary.repo_run_statuses` as primary repo truth. That export field stays a raw coordinator snapshot. When a nested child export or repo-local state is readable, report uses that child authority first and keeps coordinator linkage labels such as `linked` or `initialized` as metadata only.
