...
When `subject.run.status = "completed"` and `subject.run.run_id_mismatches` is non-empty, the report keeps those mismatches visible for audit but does not reopen recovery guidance. In that case `subject.run.terminal_observability_note` is populated, text and markdown render `Terminal drift note: ...`, HTML renders a `Terminal drift note` metadata row, and the report still omits `Next Actions`.
