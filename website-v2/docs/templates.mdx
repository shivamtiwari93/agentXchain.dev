...
| `architect` | `architecture` | `ARCHITECTURE.md` (requires `## Context`, `## Proposed Design`, `## Trade-offs`, `## Risks`) |
| `dev` | `implementation` | `IMPLEMENTATION_NOTES.md` |
| `security_reviewer` | `security_review` | `SECURITY_REVIEW.md` (requires `## Threat Model`, `## Findings`, `## Verdict`) |
...
.agentxchain/
  prompts/
    pm.md
    architect.md             ← includes owned phase, gate, and artifact context
    dev.md
    security_reviewer.md     ← includes owned phase, gate, and artifact context
    qa.md
    eng_director.md
```

The generated prompt files for custom roles (`architect.md`, `security_reviewer.md`) include their owned phase, exit gate ID, and workflow artifacts so operators can immediately see the contract without cross-referencing the config.
...
Both `ARCHITECTURE.md` and `SECURITY_REVIEW.md` are structurally validated by `template validate` via `section_check` semantics. The gate will not pass if required sections are missing.
...
#### Building your own blueprint-backed template

The `enterprise-app` manifest demonstrates the pattern. Any template can carry a `scaffold_blueprint` with custom `roles`, `runtimes`, `routing`, `gates`, and `workflow_kit`. Inspect the shipped manifest for the full contract:
