...
`enterprise-app` is intentionally not just another artifact bundle. It is an init-only blueprint-backed template with a custom governed team blueprint:

- `architect`
- `security_reviewer`
- custom phases: `architecture` and `security_review`
- explicit workflow-kit artifacts: `.planning/ARCHITECTURE.md` and `.planning/SECURITY_REVIEW.md`

This is the shipped proof that built-in governed templates are no longer locked to `pm`, `dev`, `qa`, and `eng_director`.
