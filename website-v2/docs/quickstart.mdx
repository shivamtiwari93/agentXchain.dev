...
Governed mode is staged and gate-driven. `agentxchain run` is the primary automated workflow, but the scaffold created by `init --governed -y` is manual-only by default:
