...
This is scaffold proof, not gate proof. A fresh scaffold still starts with `Approved: NO` and `## Verdict: PENDING`; the governed runtime will only open the planning/completion gates after those become `Approved: YES`, an affirmative `## Verdict:` value, and a non-placeholder `SYSTEM_SPEC.md` with real `Purpose`, `Interface`, and `Acceptance Tests` sections.
