...
- Minimum CLI version: `agentxchain 2.159.0` or newer
...
```bash
# Record a CI-failure signal — creates a detected intent and prints its id
agentxchain intake record \
  --source ci_failure \
  --category test_regression \
  --signal '{"description":"test suite regressed on main"}' \
  --evidence '{"type":"text","value":"npm test: 3 failing in auth.test.js"}'

# Then drive the intent through its lifecycle by id (every step takes --intent)
agentxchain intake triage --intent <intent_id>
agentxchain intake approve --intent <intent_id>
agentxchain intake plan --intent <intent_id>
agentxchain intake start --intent <intent_id>     # repo-local execution
```
