...
| Planning artifacts | `.planning/PM_SIGNOFF.md`, `.planning/ROADMAP.md`, `.planning/SYSTEM_SPEC.md`, `.planning/acceptance-matrix.md`, `.planning/ship-verdict.md` |
...
This is scaffold proof, not gate proof. A fresh scaffold still starts with `Approved: NO`, a placeholder acceptance matrix row, and `## Verdict: PENDING`; the governed runtime will only open the planning/completion gates after those become `Approved: YES`, a QA-owned acceptance matrix whose requirement rows all end in passing `Status` values, an affirmative `## Verdict:` value, and a non-placeholder `SYSTEM_SPEC.md` with real `Purpose`, `Interface`, and `Acceptance Tests` sections.
