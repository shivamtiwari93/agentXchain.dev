...
After the demo, read [Getting Started](/docs/getting-started) for the shortest honest copy-paste path into a real repo. Then use [Your First Governed Turn](/docs/first-turn) when you want the artifact-by-artifact breakdown.

## Before you start

- **Node.js 18.17+** (or 20.5+)
- **git** (governed scaffold expects a repo)
- `npm install -g agentxchain` for repeated commands on this page
...
- An API key for any `api_proxy` role you plan to use (`review_only` or `proposed`), such as `ANTHROPIC_API_KEY` for the default `api-qa` runtime
...
agentxchain init --governed --template web-app --dir . -y
...
**Tip:** pass `--goal "..."` at init time to set a project mission that appears in every dispatch bundle, status output, and governance report. Or set `project.goal` in `agentxchain.json` later.
