...
After the demo, read [Your First Governed Turn](/docs/first-turn) for a step-by-step walkthrough of what every artifact looks like. Then continue with Path 1, 2, or 3 below when you want to bootstrap a real repo.
...
| Planning artifacts | `.planning/PM_SIGNOFF.md`, `.planning/ROADMAP.md`, `.planning/SYSTEM_SPEC.md`, `.planning/IMPLEMENTATION_NOTES.md`, `.planning/acceptance-matrix.md`, `.planning/ship-verdict.md`, `.planning/RELEASE_NOTES.md` |
...
If you use another local tool instead of the scaffolded `claude --print --dangerously-skip-permissions` command, prefer setting it during `init` with `--dev-command` and `--dev-prompt-transport`. Editing `runtimes.local-dev` afterward still works, but the point is to scaffold a truthful runtime contract up front. See [Adapters](/docs/adapters) for the runtime contract.
...
Write your planning docs (`PM_SIGNOFF.md`, `ROADMAP.md`), then fill the staged turn result. A fresh scaffold starts with `PM_SIGNOFF.md` set to `Approved: NO`; change that line to `Approved: YES` only after a human reviews the planning artifacts and is ready to open the planning gate. Copy the exact template from the dispatch bundle instead of inventing fields. The minimum truthful shape still looks like this:
...
If QA is bound to `api_proxy` in this flow, the accepted turn produces a structured review and an orchestrator-materialized review artifact under `.agentxchain/reviews/`. It does **not** mean the QA gate files were rewritten automatically.
