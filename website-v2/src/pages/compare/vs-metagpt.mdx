...
| **Governance model** | Standard Operating Procedures (SOPs), environment/message abstractions, and app-defined coordination | Protocol-enforced turns, objections, phase gates, decision ledger |
...
| **Recovery** | Serialization/breakpoint recovery plus flow-specific redo/resume | Governed blocked-state recovery, escalation, resume |
| **Hosted product** | Atoms (formerly MGX / MetaGPT X) for hosted app building and deployment | Open-source/local-first runner today; managed `.ai` surface later |
