...
| **Quality controls** | Guardrails on agent outputs; no mandatory cross-role challenge | Mandatory protocol-enforced challenge between roles |
| **Human involvement** | `human_input_mode`, user agents, manual speaker selection | Phase-transition and run-completion gates |
| **Interoperability** | A2A and AG-UI protocol support | Connector-based (manual, local CLI, API proxy) |
| **Audit surface** | Conversation history and observability integrations | Append-only decision and objection ledgers |
