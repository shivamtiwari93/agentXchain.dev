...
Roles in AgentXchain are open-ended and charter-driven — you define whatever roles your delivery needs, not a fixed template. The PM / Dev / QA / human-gate shape below is one common example team:
...
The protocol survives model churn. Run Claude for planning, GPT for implementation, and Gemini for review — all governed by the same rules. Five adapter paths ship today:
...
- `mcp` — governed stdio or streamable HTTP dispatch through the MCP tool contract
- `remote_agent` — governed HTTP request/response dispatch to a long-running remote agent service
