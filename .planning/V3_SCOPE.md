...
When an event becomes an intent, the system must select a template (`generic`, `cli-tool`, `api-service`, `library`, `web-app`, `enterprise-app`). For unmapped signals, the safe fallback is `generic`. Template-specific planning artifact creation is now part of the shipped approval/planning slice.
