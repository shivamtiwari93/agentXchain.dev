# Launch Evidence Report — AgentXchain v2.158.0

> Single-source evidence artifact for launch-facing claims. Updated 2026-06-25 for v2.158.0 — the `ship-status` shippability summary, the `attention` govern-by-exception view, artifact-finalization acceptance after committed product code, and the single-shot execution guard for dispatched turns are now in the release train. The v2.158.0 train is staged on the current released line v2.157.0 (`- npm test -- --test-timeout=60000 -> 7706 tests / 1561 suites / 0 failures / 5 skipped`). Current aggregate release evidence:
>
