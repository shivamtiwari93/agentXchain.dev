# Launch Evidence Report — AgentXchain v2.159.0

> Single-source evidence artifact for launch-facing claims. Updated 2026-06-29 for v2.159.0 — role-charter well-formedness validation (`agentxchain role validate`) and a public-surface accuracy & light-mode pass (docs corrected against the shipped CLI, working light mode, examples hardening). The v2.159.0 train is staged on the current released line v2.158.0 (`- npm test -- --test-timeout=60000 -> 7724 tests / 1579 suites / 0 failures / 5 skipped`). Current aggregate release evidence:
>
