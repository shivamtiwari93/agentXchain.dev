# Launch Evidence Report — AgentXchain v2.151.0

> Single-source evidence artifact for launch-facing claims. Updated 2026-04-21 for the v2.151.0 architectural-fix release over `v2.150.0`. BUG-59 full-auto gate closure coupling (approval_policy ↔ phase-advance) shipped across four reviewable slices (policy primitives, schema guard, reconcile-path coupling, defaults + beta scenario + specs/docs); BUG-54 startup watchdog default raised from 30 s to 120 s after Turn 137's measurement (113 s first-stdout on 17,737-byte bundle) and a tester `stdout_attach_failed` at `running_ms: 30285` on real `tusq.dev` full-auto; BUG-59 template-manifest whitelist repair so the packaged enterprise-app scaffold accepts the new `approval_policy` defaults. BUG-59 and BUG-54 close only after tester-quoted shipped-package output on `agentxchain@2.151.0` per discipline rules #12 and #13. Current aggregate release evidence:
>
