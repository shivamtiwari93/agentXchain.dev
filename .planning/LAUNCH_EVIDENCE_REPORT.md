# Launch Evidence Report — AgentXchain v2.153.0

> Single-source evidence artifact for launch-facing claims. Updated 2026-04-22 for the v2.153.0 BUG-61 ghost-turn auto-recovery release over `v2.152.0`. BUG-61 ships the auto-retry primitive (config + CLI + full-auto promotion), the same-signature early-stop safeguard (2 consecutive identical `runtime|role|failure_type` fingerprints → stop before exhausting the raw budget), the full `ghost_retry_exhausted` diagnostic bundle payload, and the active-run continuation branch that prevents a reissued turn from being dropped back into intake/vision seeding. `hasBlockingActiveTurn()` was widened to treat `failed_start`/`stalled` as blocking. BUG-61 closes only after tester-quoted shipped-package output on `agentxchain@2.153.0`. BUG-52, BUG-54, BUG-59, BUG-53 remain open pending their own tester-quoted shipped-package output. Current aggregate release evidence:
>
