...
3. After staging, it must partition every declared path into one of three classes:
   ...
   - **already_committed_upstream**: path is tracked in git and has zero divergence against HEAD or the index. This covers the BUG-23 legitimate pattern where the actor committed a declared file before `checkpoint-turn` ran.
