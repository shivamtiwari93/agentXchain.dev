...
```json
{
  "gates": {
    "qa_ship_verdict": {
      ...
      "gate_actions": [
        { "label": "publish release", "run": "bash scripts/publish-release.sh" },
        { "label": "deploy docs", "run": "bash scripts/deploy-docs.sh" }
      ]
    }
  }
}
...
5. Approval finalization happens only after all gate actions succeed.
6. If any gate action fails:
...
7. Because retries reuse the same approval command, configured gate-action commands must be idempotent and safe to rerun.
8. Every executed gate action appends a `gate_action` ledger entry with:
   - gate id/type
   ...
   - timestamp
   - approval attempt id
9. `status` shows the latest gate-action attempt for the current pending gate.
10. `report` and `audit` include gate-action evidence from the decision ledger.
