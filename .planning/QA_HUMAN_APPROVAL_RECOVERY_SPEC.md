...
- Inference must not run while any active turn is still retained. A retained turn is the authoritative recovery target; stale `blocked_on: "human_approval:*"` metadata must not synthesize a gate wait on top of it.
...
- If any active turn is still retained, no approval pause is inferred from `blocked_on: "human_approval:*"`.
...
- `cli/test/operator-recovery.test.js`
  ...
  - `AT-QAAPP-004`: retained stale turns suppress orphaned approval inference and remain the recovery target
