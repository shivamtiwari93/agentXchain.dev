...
metadata output carries runtime id, resolved command/args, prompt transport,
bundle size, watchdog threshold, auth env snapshot (`env_snapshot.auth_env_present`),
Claude binary probe, and summary counters; the `.attempts[]` output carries
per-attempt timing. `prompt_transport` and `env_snapshot.auth_env_present`
are the diagnostic signals for the two remaining BUG-54 root-cause
hypotheses (stdin/EPIPE handling and auth env not propagating to the
Claude subprocess); missing either field in the quote-back lets a
deterministic-failure root cause hide inside a green-looking fallback.
