...
4. The page states the real recovery contract: repo-local governed state is authoritative; coordinator state can be rebuilt from it.
...
- Omitting `multi approve-gate` or `multi resync --dry-run`
...
- AT-MRD-003: The page documents all five `multi` subcommands and the repo-local `accept-turn` step that completes the operator loop.
