...
- [x] Acceptance: `agentxchain ship-status` returns structured shippability assessment composing 5 evidence dimensions; coordinator runs show per-repo ship readiness; blocking reasons surfaced when not shippable <!-- QA verified run_74d17633499b410b/turn_f26ac4b155de15b4: 6/6 acceptance criteria pass, 23/23 ship-status + 69/69 combined report-integration tests (0 failures, QA-run), live --json/--verbose compose 5 dimensions with surfaced blocking reasons; ship verdict YES. Gate re-cleared run_74d17633499b410b/turn_b7ac694416a751c0 after fixing acceptance-matrix `| Req # |` header that had rejected the prior gate request; re-ran 23/23 + 69/69 (exit 0) and confirmed all 3 artifact gate validators {ok:true}. -->

### M15: Govern Without Micromanaging — Human Attention Surface — Vision Closure (VISION.md:51)

Closes the final unaddressed "Why This Must Exist" pain bullet, **VISION.md:51 "humans lose the ability to govern without micromanaging."** The vision's "Human Role" section (VISION.md:199–220) requires humans to retain exactly five rights — set direction, define boundaries, **approve critical transitions**, **intervene during escalation**, **decide what ships** — *without* needing to "micromanage every step," and demands "lights-out operation without requiring blind trust" (VISION.md:220). Today those human-decision triggers are scattered: `status` shows only the current run's escalation + pending intents, the dashboard shows everything (the opposite of exception-based), and there is no single cross-run command that answers **"what needs MY attention right now, and nothing else?"** M15 builds a govern-by-exception composition layer that aggregates the human-decision queue across runs into one prioritized surface — and is empty when nothing needs a human, which is the operational proof of "govern without micromanaging."

- [x] New module `cli/src/lib/human-attention.js` with `evaluateHumanAttention(repoDir)` composing the human-decision queue across ≥5 exception categories — (1) pending phase-transition/completion approvals (`state.blocked_on` `human_approval:<gate>`), (2) open human escalations (`readHumanEscalations`/`findCurrentHumanEscalation`), (3) pending approved intents awaiting dispatch (`findPendingApprovedIntents`), (4) credentialed gates that require a human even in lights-out (`isCredentialedExitGate` + `evaluateApprovalPolicy`), (5) budget/policy blockers (`state.blocked_on` `budget:exhausted` / `policy:<id>`) — into a structured `HumanAttentionReport` with `overall: 'clear' | 'attention'`, a priority-ordered `items[]` (each `{ category, priority, blocking, run_id, summary, action_hint }`), and an `evidence_summary`. Read-only; composes existing modules, reimplements none.
- [x] `agentxchain attention` CLI command with `--json` and `--all` flags — single govern-by-exception operator command. Default output lists ONLY items needing a human, highest-priority first, each with a concrete `action_hint` (e.g. `agentxchain approve-transition`, `agentxchain escalate --resolve`); prints an explicit "Nothing needs your attention" line and exits 0 when the queue is empty (the micromanagement-free state).
- [x] Govern-by-exception semantics: empty queue ⇒ `overall: 'clear'` with zero items; non-empty queue ⇒ `overall: 'attention'` with deterministic priority ordering (blocking before non-blocking; credentialed-gate and escalation categories outrank informational pending-intent items).
- [x] Governance report integration — `buildGovernanceReport()` includes a `human_attention` summary section (`overall`, `items_count`, `blocking_count`, `categories[]`) so the attention queue is visible in the standard report surface.
...
- [x] New module `cli/src/lib/role-charter.js` with `evaluateRoleCharter(config, rawConfig, roleId)` and `evaluateAllRoleCharters(config, rawConfig)` that score a role against the four VISION:123–128 invariants <!-- run_dc50efa0354c0768/turn_29fc38df7a095987: dev shipped role-charter.js (evaluateRoleCharter/evaluateAllRoleCharters/buildRoleCharterSummary); 18/18 role-charter tests pass --> — (1) **mandate** present (`role.mandate` non-empty), (2) **authority boundary** coherent (`role.write_authority` valid AND its runtime binding resolves to a non-`none` `effective_write_path` via `getRoleRuntimeCapabilityContract`), (3) **produces governed artifacts** (role can reach required file production in ≥1 routed phase via `canRoleParticipateInRequiredFileProduction`, OR owns ≥1 workflow-kit artifact it can satisfy via `canRoleSatisfyWorkflowArtifactOwnership`/`getEffectiveGateArtifacts`), (4) **participates in a structured workflow** (role appears as `entry_role` or in `allowed_next_roles` of ≥1 phase in `config.routing`) — into a structured `RoleCharterReport` (`role_id`, `overall: 'well_formed' | 'incomplete'`, `invariants[]` each `{ id, name, satisfied, detail }`, `missing[]`, `evidence_summary`). Read-only; composes `runtime-capabilities.js`, `gate-evaluator.js`, `admission-control.js`, and config routing — reimplements none.
- [x] `agentxchain role validate [role_id]` subcommand (extend `cli/src/commands/role.js` + `roleCmd` in `cli/bin/agentxchain.js:958`) — validates one role, or all roles when `role_id` omitted; `--json` emits the full report(s). Exit 0 when all evaluated roles are `well_formed`; exit non-zero (1) when any evaluated role is `incomplete`, listing each unsatisfied invariant with a concrete fix hint. <!-- run_dc50efa0354c0768/turn_29fc38df7a095987: role.js validateRoles() + agentxchain.js role validate registration; AT-RC-010/011/012 + live smoke (exit 0 all-well-formed / exit 1 incomplete) pass -->
- [x] Four-invariant evaluation semantics: `overall === 'well_formed'` **iff** all four invariants satisfied; `missing[]` enumerates unsatisfied invariant ids; each invariant reports independently (one failing invariant never suppresses evaluation of the others). The reference roles (`pm`/`dev`/`qa`) and an `enterprise-app` custom role (`security_reviewer`) both evaluate `well_formed` against a valid config. <!-- run_dc50efa0354c0768/turn_29fc38df7a095987: AT-RC-007 (3 failing invariants, 4th still independently evaluated), AT-RC-009 (well_formed iff 4/4), AT-RC-001 (pm/dev/qa), AT-RC-002 (enterprise security_reviewer) pass -->
- [x] Governance report integration — `buildGovernanceReport()` (`report.js:1245`, mirroring `buildHumanAttentionSummary` at `report.js:1083`) includes a `role_charters` summary section (`total`, `well_formed`, `incomplete`, `incomplete_role_ids[]`). <!-- run_dc50efa0354c0768/turn_29fc38df7a095987: report.js imports buildRoleCharterSummary, adds role_charters to buildRunSubject after human_attention; governance-report-content + workflow-kit-report suites green -->
- [x] Regression tests `cli/test/role-charter.test.js` (AT-RC-001…013): all-well-formed config; missing-mandate; authority-boundary incoherent (review_only on local_cli → none); produces-no-artifacts (not routed to any file-producing phase and owns nothing); not-in-routing; mixed multi-role report; `well_formed iff 4/4`; `role validate` exit 0 all-well-formed; `role validate` exit 1 with an incomplete role; `--json` schema; all-roles vs single-role; read-only (config byte-identical before/after). <!-- run_dc50efa0354c0768/turn_29fc38df7a095987: 18/18 role-charter tests pass (AT-RC-001…013 + report integration + unknown-role); vitest-contract count 690→691 -->
- [x] Acceptance: `agentxchain role validate` returns a per-role four-invariant well-formedness assessment composed from existing capability/routing/artifact primitives; reference roles and an `enterprise-app` custom role pass; an intentionally malformed custom role is reported `incomplete` with its missing invariant(s) and a fix hint; exit 0 all-well-formed / exit 1 any-incomplete; governance report exposes `role_charters`; read-only; addresses VISION.md:100–130 "Roles Are Open-Ended, Not Fixed" four-part charter invariant (VISION.md:123–128). <!-- run_dc50efa0354c0768/turn_58f0ffa97741e28b: QA ship verdict YES — 18/18 role-charter + 267/267 regression batch (12 suites) green; live CLI exit 0 (4/4 well-formed) / exit 1 (charter-incomplete ghost, fix hints shown); AT-RC-002 enterprise security_reviewer well_formed; e2e export→report subject.run.role_charters={total:4,well_formed:4,incomplete:0} coexists with ship_status+human_attention; read-only AT-RC-013 -->
...
| Phase | Goal | Status |
|-------|------|--------|
| Planning | Derive M15 from VISION.md:51 "humans lose the ability to govern without micromanaging" (final "Why This Must Exist" bullet); scope `human-attention.js` govern-by-exception module + `agentxchain attention` CLI command, citing VISION.md "Human Role" (199–220) and "lights-out without blind trust" (220) | Done (run_2929265fcbabe440 / turn_fd0c9ed117cd5d23) |
| Implementation | Build `human-attention.js` module (`evaluateHumanAttention` composing ≥5 exception categories), `attention` CLI command (`--json`/`--all`), govern-by-exception ordering, report integration, regression tests | Done (run_2929265fcbabe440 / turn_9d23289f5ea9873b) — module + command + report integration shipped; 18/18 human-attention tests pass; 5 build items above checked |
| QA | Verify the attention queue composes ≥5 categories, is empty/clear when no human decision is pending, orders by priority, surfaces action hints; test coverage; ship verdict | Done (run_2929265fcbabe440 / turn_92c39cb5177586c2): 6/6 AC pass, 6 categories, 18/18 + 37/37 + 46/46 tests green, live `attention` exit 0, report integration e2e-verified; ship verdict YES. Findings: stale-artifact (10th run, fixed), OBJ-001 report summary is a 3-of-6 partial view (per-spec, non-blocking) |

---

### Completed Milestone History — M14 (closed)
...
| Implementation | Build ship-status.js module, CLI command, coordinator aggregation, report integration, regression tests | Delivered & committed by run_322ba900566dddfe dev turn_9ce54587bc5981c1 (HEAD 9fdbc1c51); 23/23 ship-status tests pass |
| QA | Verify 5 evidence dimensions compose, test coverage, ship verdict | Done (run_74d17633499b410b / turn_f26ac4b155de15b4: 6/6 AC pass, 23/23 + 69/69 tests, ship verdict YES; gate re-cleared turn_b7ac694416a751c0 after fixing the acceptance-matrix `| Req # |` header) |
