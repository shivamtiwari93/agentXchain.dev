...
- Core governed flow: `init`, `config`, `status`, `export`, `resume`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `validate`, `verify protocol`, `verify export`, `migrate`
- Governed-adjacent extensions: `template list`, `template validate`, `template set`, `role list`, `role show`, `turn show`, `plugin`, `intake`, `multi`, `dashboard`, `doctor`, `history`, `events`, `demo`, `schedule`, `report`, `restore`, `restart`, `escalate`
...
2. Every governed-scope command family listed above must appear in the command map table using its public docs shape:
   - top-level rows for `init`, `config`, `status`, `export`, `restore`, `restart`, `report`, `doctor`, `demo`, `schedule`, `history`, `events`, `validate`, `verify protocol`, `verify export`, `migrate`, `resume`, `escalate`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `plugin`, `intake`, `multi`, and `dashboard`
   - subcommand rows for `template list`, `template validate`, `template set`, `role list`, `role show`, and `turn show`
3. `verify` is documented in the command map as concrete subcommand rows (`verify protocol`, `verify export`), not as the ambiguous parent `verify`.
...
- AT-CMAP-003: `verify` is documented via concrete subcommand rows (`verify protocol`, `verify export`)
