...
2. Every governed-scope command family listed above must appear in the command map table using its public docs shape:
   - top-level rows for `init`, `config`, `status`, `export`, `restore`, `restart`, `report`, `doctor`, `demo`, `schedule`, `history`, `events`, `validate`, `verify turn`, `verify protocol`, `verify export`, `migrate`, `resume`, `escalate`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `plugin`, `intake`, `multi`, and `dashboard`
