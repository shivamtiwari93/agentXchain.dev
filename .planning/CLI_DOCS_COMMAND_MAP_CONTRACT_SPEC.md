...
- Core governed flow: `init`, `config`, `status`, `export`, `resume`, `unblock`, `inject`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `validate`, `conformance check`, `verify turn`, `verify protocol`, `verify export`, `migrate`
- Governed-adjacent extensions: `template list`, `template validate`, `template set`, `role list`, `role show`, `turn show`, `plugin`, `intake`, `multi`, `dashboard`, `doctor`, `connector check`, `history`, `diff`, `events`, `demo`, `schedule`, `report`, `restore`, `restart`, `escalate`
...
2. Every governed-scope command family listed above must appear in the command map table using its public docs shape:
  - top-level rows for `init`, `config`, `status`, `export`, `restore`, `restart`, `report`, `doctor`, `connector check`, `demo`, `schedule`, `history`, `diff`, `events`, `validate`, `conformance check`, `verify turn`, `verify protocol`, `verify export`, `migrate`, `resume`, `unblock`, `inject`, `escalate`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `plugin`, `intake`, `multi`, and `dashboard`
...
- AT-CMAP-003: `verify` is documented via concrete subcommand rows (`verify turn`, `verify protocol`, `verify export`) and protocol conformance is additionally discoverable as `conformance check`
