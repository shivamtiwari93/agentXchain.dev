...
- Core governed flow: `init`, `status`, `export`, `resume`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `validate`, `verify protocol`, `verify export`, `migrate`
...
2. Every governed-scope command family listed above must appear in the command map table using its public docs shape:
   - top-level rows for `init`, `status`, `export`, `resume`, `step`, `run`, `accept-turn`, `reject-turn`, `approve-transition`, `approve-completion`, `validate`, `verify protocol`, `verify export`, `migrate`, `plugin`, `intake`, `multi`, and `dashboard`
