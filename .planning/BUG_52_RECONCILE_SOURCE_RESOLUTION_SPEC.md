...
12. **Turn 275 extension — gate show must converge with approve-transition on standing pending gates.** When `agentxchain gate show <gate>` displays a gate whose `requires_human_approval: true`, whose `status: pending`, whose `linked_phase` equals `state.phase`, and when `state.pending_phase_transition` is `null`, the command must emit a Recovery hint naming the absent phase transition, the `agentxchain unblock <hesc_id>` resolution path for open escalations, and the follow-up `agentxchain approve-transition` command. Without the symmetric hint, `gate show` reports only `Status: pending` and sends the operator back into the approve-transition diagnostic that Turn 274 already pointed *from* `gate show` — closing the loop on both surfaces is the concrete form of BUG-52 fix requirement 3 ("the two surfaces must converge on a consistent truth").
...
- **Turn 275 gate-show symmetric hint path:** with the same standing-gate state (`phase_gate_status.planning_signoff: "pending"`, `pending_phase_transition: null`, `state.phase: "planning"`), `agentxchain gate show planning_signoff` exits zero but prints a Recovery block naming the missing transition, `agentxchain unblock <hesc_id>`, and `agentxchain approve-transition`. Negative: when `pending_phase_transition` is populated, the Recovery block must not appear.
- **Turn 276 reconciled session-checkpoint cleanup path:** if `reconcilePhaseAdvanceBeforeDispatch()` advances a now-satisfied phase gate while stale prior-phase active turn, budget reservation, dispatch directory, and `.agentxchain/session.json.active_turn_ids` entries exist, it must write a `phase_reconciled` session checkpoint before any next dispatch. The checkpoint phase must match the advanced state and must not retain stale prior-phase turn ids.
