...
9. Creates a single commit: `<semver>` (matching npm convention)
10. Creates an annotated tag: `v<semver>` with message `v<semver>`
11. Verifies commit and tag exist before exiting
12. Exit code 0 only if all steps succeed; any failure is fatal
...
14. Print success with commit SHA and tag
