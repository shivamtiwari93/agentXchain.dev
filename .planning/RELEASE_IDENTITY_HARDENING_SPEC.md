...
10. Creates a single commit with subject `<semver>` (matching npm convention) and body trailer `Co-Authored-By: <value from --coauthored-by>`
...
10. `git commit -m "${TARGET}\n\nCo-Authored-By: ${COAUTHORED_BY}"`
11. `git tag -a "v${TARGET}" -m "v${TARGET}"`
12. Verify: `git rev-parse "v${TARGET}"` succeeds
13. Verify: `git log -1 --format=%s` equals `${TARGET}`
14. Verify: `git log -1 --format=%B` contains `Co-Authored-By: ${COAUTHORED_BY}`
