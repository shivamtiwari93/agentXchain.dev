...
3. **The release-identity commit can exclude the already-validated target-version public surfaces.** The repo validates changelog, release notes, homepage truth, capabilities, implementor-guide examples, launch evidence, marketing drafts, and release discoverability through shared release-alignment checks. If `release-bump.sh` stages only a subset of those surfaces, operators are forced into temporary prep commits or a final release commit that does not actually contain the validated public truth.
...
```bash
bash scripts/release-bump.sh --target-version <semver> --coauthored-by "Name <email>"
```

**Contract:**
1. Validates semver format
2. Supports safe re-entry when `cli/package.json` is already at target version but no tag exists yet
...
7. Normalizes every `website-v2/docs/releases/v*.mdx` `sidebar_position` so autogenerated docs sidebars stay newest-first with unique contiguous positions
8. Stages those allowed release-surface files together with `package.json`, `package-lock.json`, and any release-note docs touched by normalization
9. Requires `--coauthored-by "Name <email>"` so the release commit carries a real trailer without post-hoc amend steps
10. Creates a single commit with subject `<semver>` (matching npm convention) and body trailer `Co-Authored-By: <value from --coauthored-by>`; in re-entry mode it either reuses the existing `HEAD` commit when that commit already matches the subject+trailer contract, or creates a metadata-only release identity commit that records `Release-Base: <prior HEAD>` when the tree is already clean and aligned
11. Creates an annotated tag: `v<semver>` with message `v<semver>`
12. Verifies commit subject, commit trailer, and tag exist before exiting
13. Exit code 0 only if all steps succeed; any failure is fatal
...
1. Parse `--target-version <semver>` and `--coauthored-by "Name <email>"` (both required)
2. `cd` to cli directory (script is in `cli/scripts/`)
3. Read the current package version and enter **re-entry mode** when it already matches the target version
4. Assert the tree contains no dirty paths outside the release-surface whitelist for the target version; in re-entry mode, allow `cli/package.json` and `cli/package-lock.json` dirt as part of the pending release identity
5. Fail closed unless every governed version surface already references the target version (current public/docs surfaces pre-bump; marketing/package-version surfaces stage into the same release commit)
6. Normalize all release-note `sidebar_position` values so reverse-semver order maps to contiguous positions starting at `0`
...
8. Run `npm version ${TARGET} --no-git-tag-version`; in re-entry mode, use `--allow-same-version` so `package-lock.json` is normalized without forcing a manual tag workaround
...
10. If staged changes remain, `git commit -m "${TARGET}\n\nCo-Authored-By: ${COAUTHORED_BY}"`; otherwise:
    - if `HEAD` already has subject `${TARGET}` and the required `Co-Authored-By` trailer, reuse it
    - else if re-entry mode is active, create `git commit --allow-empty -m "${TARGET}\n\nRelease-Base: <prior HEAD>\nCo-Authored-By: ${COAUTHORED_BY}"` so the exact clean tree receives an explicit release identity without inventing file churn
    - else fail closed
11. `git tag -a "v${TARGET}" -m "v${TARGET}"`
12. Verify: `git rev-parse "v${TARGET}"` succeeds
13. Verify: the release commit subject equals `${TARGET}`
14. Verify: the release commit body contains `Co-Authored-By: ${COAUTHORED_BY}`
15. Print success with commit SHA and tag

### Error Cases

- Invalid semver → exit 1 with message
- Missing `--coauthored-by` trailer value → exit 1 with message
- Dirty tree outside the release-surface whitelist → exit 1 with message naming the offending files
- Already at target version, `HEAD` already claims subject `<semver>`, but the required `Co-Authored-By` trailer is missing → exit 1 with message
...
- `AT-RIH-010`: In a temp git repo with target-version discovery-surface edits prepared, `release-bump.sh --target-version <semver>` stages `website-v2/static/llms.txt` into the same release commit and rejects the bump if it omits the current release route
- `AT-RIH-011`: `release-bump.sh` automatically normalizes release-note `sidebar_position` values, strips duplicate manual `sidebar_position` lines, assigns the new release `0`, increments older releases, and stages any touched legacy release-note docs in the same release commit
...
- `AT-RIH-014`: If the repo is already at the target version and no tag exists yet, `release-bump.sh` must support re-entry safely: create a metadata-only `<semver>` release identity commit with `Release-Base: <prior HEAD>` when the tree is already clean/aligned, and reuse an existing valid `<semver>` release commit when one already exists
- `AT-RIH-015`: If `HEAD` already claims subject `<semver>` during re-entry but the required `Co-Authored-By` trailer is missing, `release-bump.sh` must fail closed instead of tagging or silently minting a second release identity
