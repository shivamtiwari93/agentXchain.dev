...
- [ ] **🚨 BUG-94: retained `failed_acceptance` dev turn emits an otherwise valid staged result but omits the required top-level `decisions` and `objections` arrays, causing schema validation to fail before authoritative-role protocol validation can accept empty arrays.** Discovered 2026-04-27 on the real `tusq.dev` DOGFOOD-100 session (`cont-a2567aec`, run `run_533b2f8c47cc0bf0`, turn `turn_60ca77d51809c98f`, dev role) after `agentxchain@2.155.47` reverified BUG-91/92/93 and advanced the strict same-session counter to 11. Observed error: `acceptTurn(dev): Validation failed at stage schema: Missing required field: decisions; Missing required field: objections`.
  ...
  **Closure criteria:**
  ...
  6. Same-session evidence exists under `.planning/dogfood-100-turn-evidence/bug-94-reverify-vX.Y.Z.md`, and the strict counter continues from counter_value 12 if the same continuous session `cont-a2567aec` is preserved.
