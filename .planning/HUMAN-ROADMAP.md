...
**Latest blocker evidence update — 2026-04-30:** `agentxchain@2.155.72` now exposes the DOGFOOD Claude credential smoke as a direct shipped bin: `npx --yes -p agentxchain@2.155.72 -c 'agentxchain-dogfood-claude-smoke --credential-env-file <agentxchain-repo>/.env --cwd <tusq-repo> --json'`. Running that exact published-package path against `tusq.dev` returned `classification:"anthropic_auth_failed"` with Anthropic provider 401 using the loaded environment. DOGFOOD-100 therefore remains paused at counter `97 / 100`, and agents must not mutate Tusq state until this shipped-package smoke returns `classification:"success"`.
