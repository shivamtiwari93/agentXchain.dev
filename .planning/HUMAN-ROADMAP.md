...
- [ ] **🚨 BUG-89: QA turn emits an objection `id` that does not match `OBJ-NNN`; AgentXchain correctly rejects schema-invalid staged output, but the continuous dogfood session pauses and requires operator recovery instead of automatic normalization/retry.** Discovered 2026-04-26 on the same `tusq.dev` DOGFOOD-100 session after BUG-88 was reverified on shipped `agentxchain@2.155.42`. The blocker is in run `run_e40832d436a42d75`, turn `turn_4125be3cf057395a` (`qa`), with: `acceptTurn(qa): Validation failed at stage schema: objections[0].id must match pattern OBJ-NNN.`
