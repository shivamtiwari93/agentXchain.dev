...
- [ ] **BUG-64 (dogfood / idle-expansion acceptance contract): PM idle-expansion turns can produce the required structured result as `.agentxchain/staging/<turn>/idle-expansion-result.json`, but `acceptTurn()` only accepts top-level `turnResult.idle_expansion_result` and blocks the run with `idle_expansion_result is required for vision_idle_expansion turns`.** Dogfood reproduced on shipped `agentxchain@2.155.3` against `tusq.dev-agentxchain-dogfood`: session `cont-7182a188`, run `run_ce89ef5bd4b8cca8`, turn `turn_e614e7a53ef67b3a`, intent `intent_1777046032635_2eab`. Evidence: `.planning/dogfood-tusq-dev-evidence/GAP-003-bug64-idle-expansion-sidecar.md`. Required fix: `validateStagedTurnResult()` must load a sibling `idle-expansion-result.json` for required `vision_idle_expansion` turns when the top-level field is absent, normalize dogfood sidecar fields (`proposed_intent`, string `vision_traceability`) into canonical `idle_expansion_result`, and still run the canonical idle-expansion validator. Rule #13 coverage required: positive sidecar acceptance, positive dogfood-shape normalization, and existing negative missing-result validation must remain red when neither top-level nor sidecar result exists. Local fix implemented with focused tests (`107 tests / 107 pass` across `turn-result-validator` + BUG-60 suite); remains unchecked until shipped-package dogfood retry proves the blocked tusq.dev turn can be accepted without manual JSON surgery.
