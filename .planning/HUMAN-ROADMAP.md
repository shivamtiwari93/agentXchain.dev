...
Current focus: **🚨🚨🚨 STOP ALL FEATURE WORK. DOGFOOD-100-TURNS-CLEAN-FULL-AUTO IS THE ONLY PRIORITY. 🚨🚨🚨** Agents must use the shipped agentxchain CLI framework ONLY to drive tusq.dev development for at least **100 consecutive full-auto turns** without human intervention, manual staging-JSON edits, jq recovery surgery, or operator-side workarounds. Every issue discovered during dogfood must follow the six-step loop: discovered through dogfood failure, triaged into a BUG entry, fixed in the substrate, shipped as a patch release, reverified by resuming the dogfood state on the shipped package, and closed. **BUG-108 is open after agentxchain@2.155.61: the BUG-107 recovery path works, but a terminal dirty-baseline assignment blocker is re-recovered in a tight loop instead of remaining paused. BUG-107 remains open until shipped-package reverify reaches natural QA dispatch without a terminal blocker. BUG-78 remains open for natural no-edit review reverification.** **BUG-95, BUG-96, and BUG-97 are closed on agentxchain@2.155.51 with tusq.dev reverify evidence; BUG-98 is closed on agentxchain@2.155.52; BUG-99 is closed on agentxchain@2.155.53; BUG-100 is closed on agentxchain@2.155.54; BUG-101 is closed on agentxchain@2.155.55; BUG-102 is closed on agentxchain@2.155.56; BUG-103 is closed on agentxchain@2.155.59; BUG-104 is closed on agentxchain@2.155.59; BUG-105 is closed on agentxchain@2.155.59; BUG-106 is closed on agentxchain@2.155.60.** The v2.155.51 shipped-package run created continuous session cont-f57240d1 and accepted retained turn `turn_48fcfc7526b370ab`, so strict DOGFOOD-100 counting reset and recorded counter value 1; v2.155.53 later accepted retained QA `turn_c640f6d66166bb52`, QA `turn_57eb341ae6421d17`, launch dev `turn_c6a5bea555d5e01f`, and new-run PM `turn_99cd4b15e01e46ad`, raising the strict counter to 5 before BUG-100 blocked implementation dev turn `turn_317ed718994e61ef`. v2.155.54 resumed the blocker without `unblock`, accepted reissued dev `turn_beac02a98d4b562d`, and continued cleanly through counter value 13 before BUG-101 blocked dev turn `turn_cc1f4a9f48f528e8`. v2.155.55 resumed the BUG-101 failed-acceptance turn without manual recovery, but the continuous session ID changed to cont-6b2b572e, so the strict counter reset and advanced to 10 before BUG-102. v2.155.56 resumed BUG-102, created continuous session cont-e5fb63e4 (strict counter reset), accepted retained QA `turn_cbc4204c2b1db778` and launch dev `turn_8dbab64275e52617`, then naturally exercised BUG-77's roadmap-replenishment path before BUG-103 blocked PM turn `turn_644dcda246f21bc1`; v2.155.57 proved BUG-103 normalization advances the same turn past schema and then exposed BUG-104 at verification validation; v2.155.58 proved BUG-104 structured-evidence normalization advances the same turn past verification and then exposed BUG-105 at strict intent coverage; v2.155.59 accepted the retained PM turn end to end, completed `run_6e53e7b50cd2c457`, naturally started roadmap-replenishment `run_c39bd102a520411b`, and accepted PM `turn_400dc74e4496c4df`, closing BUG-77. The v2.155.59 invocation created live continuous session cont-9a2697e7, so strict counting reset and recorded counter values 1-4 before graceful operator stop intent; the post-stop PM turn is BUG-77 closure evidence but does not count toward DOGFOOD-100. v2.155.60 created live strict session `cont-7dc5b5df`, reached counter value 89, then discovered BUG-107; v2.155.61 preserved and recovered that session but discovered BUG-108 before QA dispatch. Until final 100-turn evidence exists, do NOT pick up watch-mode extensions, conformant-runner examples, comparison-page work, connector adoption proofs, website polish, or release-process improvements unrelated to dogfood-discovered bugs. This is the substrate-credibility gate for the whole project. If the framework cannot drive its own beta tester product through 100 clean turns, none of the other adoption surfaces matter. **Prior queue closures remain archived in .planning/HUMAN-ROADMAP-ARCHIVE.md.**
...
- [ ] **🚨🚨🚨 DOGFOOD-100-TURNS-CLEAN-FULL-AUTO: agents use the shipped `agentxchain` CLI framework ONLY to drive `tusq.dev` development for at least 100 consecutive full-auto turns with zero human intervention, zero manual staging-JSON edits, zero `jq` recovery, zero operator workarounds.** This is the substrate-credibility gate. Until 100 clean turns land on a single tusq.dev dogfood session, **all other work streams are paused** — no watch-mode extensions, no conformant-runner examples, no new comparison pages, no connector adoption, no website polish, no docs work unrelated to dogfood-discovered defects. **The output of this directive is bug-discovery + substrate hardening, not feature shipping.**
  ...
  **Currently open BUG-78, BUG-107, and BUG-108 are part of this directive's discovery output, not separate work streams.** BUG-78 remains open for natural reverification while DOGFOOD-100 continues. BUG-107 and BUG-108 block DOGFOOD-100 continuation until shipped and reverified. Closing follows the six-step discovery loop above. BUG-76, BUG-77, and BUG-79 through BUG-106 are closed with shipped-package evidence.
