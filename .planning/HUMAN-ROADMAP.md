...
- [x] **BUG-52: Phase-gate resolution doesn't advance phase — `unblock` redispatches same-phase role instead of transitioning** — ✅ **CLOSED 2026-04-23 with tester-verified shipped-package evidence on `agentxchain@2.154.11`.** This bug had a real false closure on v2.147.0, but the final downstream fix landed incrementally across `2.154.9` (state progression), `2.154.10` (continuous/full-auto continuation + launch-ready completion), and `2.154.11` (terminal accounting/audit parity). Final tusq.dev quote-back from a fresh clone proved the full governed path now completes correctly: final state `status=completed`, `phase=launch`, all four gate states `passed`, `active_turns={}`, `pending_run_completion=null`, `blocked_on=null`; final terminal events include both `gate_approved` with `gate_type: "run_completion"` and `run_completed`; `.agentxchain/continuous-session.json` now ends at `status: "completed"`, `runs_completed: 1`, `idle_cycles: 0`, `completed_via: "reconciled_before_dispatch"`. Canonical tester evidence bundle: `/tmp/tusq-agentxchain-quoteback-20260423-215411/08-final-proof.txt`.
  ...
  - **Closure path + shipped proof:** Turn 274 shipped the original command-surface convergence for phase advance, but tester quote-back showed the real downstream path still needed follow-on fixes. `agentxchain@2.154.9` fixed the core BUG-52 state behavior (`unblock` now advances to `implementation` with `planning_signoff=passed` and no PM redispatch). `agentxchain@2.154.10` fixed the downstream continuation/completion defects exposed once BUG-52 stopped blocking progression. `agentxchain@2.154.11` fixed the remaining accounting/audit gap so the final unblock emits `gate_approved` before `run_completed` and the continuous-session ledger reflects completion instead of stale paused state. BUG-52 is now closed because literal tester quote-back landed on the published package, not because the earlier theory or the earlier unit-shaped test happened to pass.
