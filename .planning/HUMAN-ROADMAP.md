...
- [ ] **BUG-65: Framework report artifacts pollute next turn's dispatch baseline.** After a turn acceptance fails, the framework generates `.agentxchain/reports/report-run_*.md`. This file appears as an undeclared file change for the NEXT turn, blocking acceptance with `undeclared_verification_outputs`. The workaround is manual: commit framework-generated reports before the next dispatch. The fix should either gitignore report artifacts, auto-commit them as framework housekeeping, or exclude `.agentxchain/reports/` from the undeclared-outputs check. Discovered during tusq.dev dogfood session 2026-04-24 (Turn 6 secondary issue 1). Scope: small, isolated to the acceptance validator's file-change detection.

- [ ] **BUG-66: Reissued turns produce near-identical content, triggering overlap acceptance conflicts.** When a turn fails and is reissued (`agentxchain reissue-turn`), the replacement turn often produces substantially similar content to the failed turn's partial output. The acceptance validator detects this as suspicious overlap and blocks with a conflict. Workaround: `--resolution human_merge`. The fix should either auto-detect reissued turns and relax overlap thresholds, or clean up the failed turn's partial artifacts before dispatching the replacement. Discovered during tusq.dev dogfood session 2026-04-24. Scope: medium — touches the acceptance validator's overlap detection and the reissue-turn command's cleanup responsibility.

- [ ] **BUG-67: Report generation crashes with "Invalid string length" after many turn attempts.** After a governed run accumulates many turns (successful and failed), `events.jsonl` and `history.jsonl` grow large enough that report generation crashes with a V8 string length limit error. The failure is non-blocking (governed execution continues) but reduces operator observability. The fix should stream or truncate report inputs, or limit the event window to recent-run events only. Discovered during tusq.dev dogfood session 2026-04-24. Scope: small, isolated to report generation.

- [ ] **BUG-68: `pm-idle-expansion.md` scaffold is dead code — never loaded during dispatch.** `agentxchain init --governed` scaffolds `.agentxchain/prompts/pm-idle-expansion.md` with detailed idle-expansion instructions, but the dispatch mechanism (`dispatch-bundle.js`) never loads this file. It always uses the standard `pm.md` prompt. The idle-expansion charter in `continuous-run.js` carries the real instructions inline. Either the dispatch should load `pm-idle-expansion.md` as a prompt override for idle-expansion turns, or the scaffold should stop generating a file that operators might customize but the framework ignores. Discovered during tusq.dev dogfood session 2026-04-24 (DEC-CLAUDE-T6-002). Scope: small — prompt loading path or scaffold cleanup.
