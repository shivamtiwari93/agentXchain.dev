...
4. When both repo-local and coordinator data exist for a turn, render separate titled sections ("Repo Hook Audit Log" / "Coordinator Hook Audit Log" and "Repo Hook Annotations" / "Coordinator Hook Annotations") — same pattern as the Hooks view
5. When only one source exists, render without prefix labels — no visual change for repo-only workspaces

## Acceptance Tests

- AT-TCH-001: Timeline with coordinator audit renders "Coordinator Hook Audit Log" section
- AT-TCH-002: Timeline with coordinator annotations renders "Coordinator Annotations" section
- AT-TCH-003: Timeline with both repo and coordinator audit renders separate "Repo Hook Audit Log" / "Coordinator Hook Audit Log" sections
