...
- **Post-publish:** `cli/scripts/sync-homebrew.sh` corrects the registry SHA (registry tarballs are not byte-identical to local `npm pack`). **Do NOT declare a release complete from `sync-homebrew.sh` alone — its success banner is only the Phase 2 → Phase 3 transition, not public-install proof.** Finish the release by running ONE of:
  - `bash cli/scripts/verify-post-publish.sh --target-version <semver>` for the manual/operator path (runs `sync-homebrew.sh`, then repo-mirror SHA proof, then isolated public `npx` smoke, then full test suite). See `DEC-VERIFY-POST-PUBLISH-NPX-001`.
  - `bash cli/scripts/release-downstream-truth.sh --target-version <semver>` for the CI-equivalent path, which presumes `release-postflight.sh` has already run the npx smoke earlier in the workflow.
- `npm run sync:homebrew` and direct `bash cli/scripts/sync-homebrew.sh` invocations are intentionally NOT a one-step release completion. Operators should still verify via the wrappers above, plus `npm view agentxchain version` and `gh release view vX.Y.Z`. See `DEC-HOMEBREW-SYNC-LOOPHOLE-CLOSE-001`.
