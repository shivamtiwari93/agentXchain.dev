# Spec: Comparison Page Runtime Surface Claims

## Purpose

Freeze the public comparison-page contract for how AgentXchain's runtime surface is described. Comparison pages may summarize the product, but they must not collapse the shipped connector/runtime surface into stale subsets that under-report what AgentXchain supports today.
...
3. `AT-COMPARE-RUNTIME-003`: `cd website-v2 && npm run build` succeeds after the comparison pages are updated.
