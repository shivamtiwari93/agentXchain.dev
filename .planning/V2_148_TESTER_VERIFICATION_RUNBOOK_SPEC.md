...
- The section must state the evidence needed to close each still-open item in the release lane:
  ...
  - `BUG-55 combined tester shape`: when the same QA turn has both declared `files_changed` and verification-produced fixture outputs, `accept-turn` followed by `checkpoint-turn` must still end with `git status --short` clean. The contract must say that clean tree means BUG-55 is fixed for the tester's exact reproduction.
...
3. The section names `startup_latency_ms`, `elapsed_since_spawn_ms`, `undeclared_verification_outputs`, `verification.produced_files`, `git status --short`, `phase_entered`, `reconciled_before_dispatch`, `session_continuation`, and the combined BUG-55 `accept-turn` followed by `checkpoint-turn` clean-tree proof.
