...
The unattended proof is also wired into GitHub Actions via `.github/workflows/governed-todo-app-proof.yml` on a nightly schedule and manual `workflow_dispatch` reruns, so the public claim is backed by continuous automation rather than a one-off local run.
