...
describe('Conformance count surfaces stay aligned', () => {
  ...
  it('homepage stat and architecture copy match the real fixture corpus size', () => {
    assert.equal(totalFixtures, 74, 'update this guard when the shipped corpus size changes intentionally');
  ...
  it('marketing drafts use the current corpus size', () => {
    assert.match(redditDrafts, /\b74 conformance fixtures\b/);
    assert.match(twitterThread, /\b74 conformance fixtures\b/);
  });
});
