...
describe('Conformance count surfaces stay aligned', () => {
  ...
  it('homepage stat and architecture copy match the real fixture corpus size', () => {
    assert.equal(totalFixtures, 102, 'update this guard when the shipped corpus size changes intentionally');
  ...
  it('implementor guide tier counts match the shipped corpus', () => {
    assert.match(guide, /\| `1` \| Core constitutional behavior .* \| `71` \|/);
  ...
  it('marketing drafts use the current corpus size', () => {
    assert.match(redditDrafts, /\b102 conformance fixtures\b/);
    assert.match(twitterThread, /\b102 conformance fixtures\b/);
  });
});
